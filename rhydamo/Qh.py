...
class Qh_relations:
    ...
    def create_Qh_relations(self, nc_file_paths, TRC_basin_relation, n_average_samples):
        """
        Concatenates Qh relations from multiple netCDF files into a single DataFrame.

        Parameters:
            nc_file_paths (list): List of file paths to netCDF files containing Qh data.
            TRC_basin_relation (DataFrame): DataFrame mapping TRC values to basin information.
            n_average_samples (int): Number of last samples to average when computing Qh relations.

        Returns:
            DataFrame: A concatenated DataFrame containing Qh relations with columns: 'discharge', 'level',
                        'node_id', and 'n_simulation'. The 'n_simulation' column indicates the simulation number.
                        This may be useful for aggregation purposes later.
        """
    ...
    def add_zero_point(Qh_per_node_id):
        """
        Extrapolates the value of level to the zero point (discharge = 0) for a given Qh relation per node_id.

        This function calculates a linear extrapolation of level based on the two lowest discharge points available
        in the input DataFrame Qh_per_node_id. It assumes a linear relationship between discharge and level
        and extrapolates the level value to the point where discharge is zero.

        Parameters:
            Qh_per_node_id (DataFrame): A DataFrame containing 'discharge' and 'level' columns sorted by 'node_id' 
                                        and 'level'.

        Returns:
            float: The extrapolated 'level' value at discharge = 0 for the given node_id.

        Note:
            This function expects the input DataFrame to be sorted by 'node_id' and 'level' in ascending order.
            If there are not enough data points to perform the extrapolation (less than 2 rows for a node_id), 
    ...
    def Qh_post_process(self, Qh, minimum_threshold, fill_value):
        ...
        """
        Post-processes Qh relations to ensure discharges meet specified criteria, are ascending, and adds extrapolation points.

        This function performs several post-processing steps on the input Qh relations DataFrame:
        1. Removes rows with discharges below a minimum threshold.
        2. Ensures that discharges and levels are in ascending order within each node_id group.
        3. Adds extrapolation points for Q = 0 for each node_id.

        Parameters:
            Qh (DataFrame): A DataFrame containing 'discharge', 'level', 'node_id', 'n_simulation', and 'row_id' columns.
            minimum_threshold (float): The minimum allowable discharge value. Rows with discharges below this threshold will be removed.
            fill_value (float): The value to fill discharges and levels when adjusting values below the threshold.

        Returns:
            Qh (DataFrame): The post-processed Qh relations DataFrame.
            removed_indexes (tuple): A tuple containing two lists of row indexes removed during processing: (indexes_to_remove_threshold, indexes_to_remove_ascending).
            altered_node_ids (list): A list of node_ids for which discharge values were adjusted.
        ...
        for count in range(len(Qh.node_id.unique())): #loop through each unique Qh relation
            ...
            if len(rows_to_remove)>0: 
                indexes_to_remove_threshold.append(rows_to_remove)


        indexes_to_remove_threshold = np.concatenate(indexes_to_remove_threshold) #flatten
        Qh = Qh.loc[~Qh.row_id.isin(indexes_to_remove_threshold)]    


    #now remove non ascending values    
        print('- Removing non ascending discharges and levels')

        indexes_to_remove_ascending = []    
        ...
        for i in range(len(Qh)-1):
            ...
            if (previous_discharge_value >= new_discharge_value or previous_level_value >=new_level_value) and previous_nodeid_value == new_nodeid_value:
                ...
                if length_nodeid - count > 2: #at least two rows should remain. 
                    indexes_to_remove_ascending.append(row_id)
                else:
        ...
        Qh = Qh.loc[~Qh.row_id.isin(indexes_to_remove_ascending)]   
        ...
        return Qh, (indexes_to_remove_threshold, indexes_to_remove_ascending), alterated_Qh_index

    
    def set_minimum_dt_Qh(self, Qh, dh_Qh_relation, changed_Qh_index, Terminal):
        """
        Adjusts the 'level' values in Qh relations to meet a minimum elevation change requirement (dh_Qh_relation).

        This function modifies the 'level' values in the Qh relations DataFrame (Qh) to ensure that the elevation change
        (Delta_h) between the highest and lowest 'level' values for each node_id is at least dh_Qh_relation. 

        Parameters:
            Qh (DataFrame): A DataFrame containing 'discharge', 'level', 'node_id', and 'level_coord' columns.
            dh_Qh_relation (float): The minimum allowable elevation change between the highest and lowest 'level' values.
            changed_Qh_index (list): A list of indexes indicating the rows in Qh that have been previously modified.

        Returns:
            Qh (DataFrame): The Qh relations DataFrame with adjusted 'level' values.
            new_manning_nodes (GeoDataFrame): A GeoDataFrame containing rows from Qh that were adjusted to meet the elevation change requirement.
            alterated_Qh (GeoDataFrame): A GeoDataFrame containing rows from Qh that were previously modified.
        ...
        lowest_Qh_post_processed = Qh.drop_duplicates(keep='first', subset = 'node_id').reset_index(drop=True)
        highest_Qh_post_processed = Qh.drop_duplicates(keep='last', subset = 'node_id').reset_index(drop=True)

        Delta_h = highest_Qh_post_processed['level'] - lowest_Qh_post_processed['level']
        Delta_h_low = Delta_h[Delta_h<dh_Qh_relation]
        
        Delta_h_low = Delta_h_low[~Delta_h_low.index.isin(Terminal.node_id)] #avoid the edges to the terminal becoming a manning resistance
        ...
        for node_id in Delta_h_low_node_id:
            matching_rows = Qh[Qh['node_id'] == node_id]
            if not matching_rows.empty:
                last_index = matching_rows.index[-1]
                Qh.at[last_index, 'level'] += dh_Qh_relation
                
                        
        alterated_Qh = Qh.iloc[alterated_Qh_index]
        Qh_post_processed_plot = gpd.GeoDataFrame(Qh, geometry = 'level_coord')
        alterated_Qh = gpd.GeoDataFrame(alterated_Qh, geometry = 'level_coord')

        new_manning_nodes = Qh.loc[Qh.node_id.isin(alterated_Qh_index)]
        ...
        return Qh, new_manning_nodes, alterated_Qh
    
    def last_check_Qh(self, Qh, fill_value):
        """
        Performs a final check and adjustment on Qh relations to ensure ascending 'discharge' and 'level' values.

        This function performs a last check on the Qh relations DataFrame (Qh) to ensure that both 'discharge' and 'level'
        values are in ascending order within each 'node_id' group. If it detects non-ascending values, it adjusts 'level'
        values to maintain the ascending order. This is determined by the fill_value

        Parameters:
            Qh (DataFrame): A DataFrame containing 'discharge', 'level', 'node_id', and 'row_id' columns.
            fill_value (float): The value used to adjust 'level' values when they violate the ascending order condition.

        Returns:
            Qh (DataFrame): The Qh relations DataFrame with adjusted 'level' values to maintain ascending order.

        """
            
        print('- Perform the last check on the Qh relations.')        
        indexes_to_remove_lastcheck = []    
        ...
        for i in range(len(Qh)-1):
            ...
            if (previous_discharge_value >= new_discharge_value or previous_level_value >=new_level_value) and previous_nodeid_value == new_nodeid_value:
                ...
                if length_nodeid - count > 2: #at least two rows should remain. 
                    indexes_to_remove_lastcheck.append(row_id)
                else:
                    Qh.loc[Qh.row_id == row_id, 'level'] += fill_value #avoid double level values
            else:
                previous_discharge_value = new_discharge_value
                previous_level_value = new_level_value
                previous_nodeid_value = new_nodeid_value
        ...
        Qh = Qh.loc[~Qh.row_id.isin(indexes_to_remove_lastcheck)] 
